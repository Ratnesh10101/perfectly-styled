...
import { ArrowRight, DraftingCompass, LogIn } from "lucide-react";
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
           ...
           <p className="text-muted-foreground">
            Get your comprehensive style report for just <strong className="text-primary">£19.99</strong>.
          </p>
        </CardContent>
          ...
          {currentUser ? (
            userMeta?.hasGeneratedReport ? (
              <Button asChild size="lg">
                <Link href="/report">View Your Report <ArrowRight className="ml-2 h-5 w-5" /></Link>
              </Button>
            ) : userMeta?.questionnaireComplete ? (
               <Button asChild size="lg">
                <Link href="/payment">Proceed to Payment <ArrowRight className="ml-2 h-5 w-5" /></Link>
              </Button>
            ) : (
              <Button asChild size="lg">
                <Link href="/questionnaire">Start Your Style Questionnaire <DraftingCompass className="ml-2 h-5 w-5" /></Link>
              </Button>
            )
          ) : (
            <>
